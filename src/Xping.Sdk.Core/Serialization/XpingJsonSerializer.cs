...
public sealed class XpingJsonSerializer : IXpingSerializer
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="XpingJsonSerializer"/> class with API options.
    /// </summary>
    public XpingJsonSerializer()
        : this(XpingSerializerOptions.ApiOptions)
    {
    }
    ...
    public T? Deserialize<T>(ReadOnlySpan<byte> utf8Json)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Failed to deserialize UTF-8 JSON to type {typeof(T).Name}.",
                ex);
        }
    }
    ...
    /// <inheritdoc/>
    /// <remarks>
    /// This method does NOT dispose the stream. The caller retains ownership and must dispose it.
    ...
    public async Task<T?> DeserializeAsync<T>(Stream stream, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Failed to deserialize stream to type {typeof(T).Name}.",
                ex);
        }
    }
}
